...
public class CookieHelper {
	...
	public static void addCookie(HttpServletResponse response, String domain, String path, String name, String value,
		...
		/** Cookie 只读设置 */
		if (httpOnly) {
			CookieHelper.addHttpOnlyCookie(response, cookie);
		} else {
			/*
	...
	public static void clearAllCookie(HttpServletRequest request, HttpServletResponse response, String domain,
		...
		for (Cookie cookie : cookies) {
			CookieHelper.clearCookie(response, cookie.getName(), domain, path);
		}
		CookieHelper.logger.info("clearAllCookie in  domain " + domain);
	}
	...
	private static boolean clearCookie(HttpServletResponse response, String cookieName, String domain, String path) {
		...
		try {
			...
			cookie.setMaxAge(CookieHelper.CLEAR_IMMEDIATELY_REMOVE);
			cookie.setDomain(domain);
			cookie.setPath(path);
			response.addCookie(cookie);
			CookieHelper.logger.fine("clear cookie " + cookieName);
			result = true;
		} catch (Exception e) {
			CookieHelper.logger.severe("clear cookie " + cookieName + " is exception!\n" + e.toString());
		}
		return result;
	}
