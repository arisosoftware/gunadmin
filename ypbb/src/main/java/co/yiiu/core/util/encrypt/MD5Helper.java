...
public class MD5Helper {
	...
	static {
		try {
			MD5Helper.messagedigest = MessageDigest.getInstance("MD5");
	...
	private static void appendHexPair(byte bt, StringBuffer stringbuffer) {
		char c0 = MD5Helper.hexDigits[(bt & 0xf0) >> 4];
		char c1 = MD5Helper.hexDigits[bt & 0xf];
	...
	public static String getFileMD5String(File file) throws IOException {
		...
		MD5Helper.messagedigest.update(byteBuffer);
		return MD5Helper.bufferToHex(MD5Helper.messagedigest.digest());
	}

	private static String getMD5String(byte[] bytes) {
		MD5Helper.messagedigest.update(bytes);
		return MD5Helper.bufferToHex(MD5Helper.messagedigest.digest());
	}
	...
	public static String getMD5String(String str) {
		return MD5Helper.getMD5String(str.getBytes());
	}
	...
	public static String getMD5StringWithSalt(String password, String salt) {
		...
		return MD5Helper.getMD5String(password + "{" + salt.toString() + "}");
	}
	...
	public static String hexdigest(byte[] paramArrayOfByte) {
		try {
			...
			for (;;) {
				...
				int k = arrayOfByte[i];
				int m = j + 1;
				arrayOfChar[j] = MD5Helper.hexDigits[(0xF & k >>> 4)];
				j = m + 1;
				arrayOfChar[m] = MD5Helper.hexDigits[(k & 0xF)];
				i++;
			}
